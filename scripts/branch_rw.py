...
class Branched_1D_Walkers:
        ...
        if not self.stationary:
            if self.gaussian:
                ...
                return np.random.choice(
                    np.array([-self.jump, self.jump]), (3 * self.rows, self.jump_rate)
                )
        else:
            if self.gaussian:
                return np.random.normal(
                    0,
                    self.jump,
                    (self.rows + 2 * (1 - int(self.count_link)) * self.row_add,),
                )
            else:
                return np.random.choice(
                    np.array([-self.jump, self.jump]),
                    (self.rows + 2 * (1 - int(self.count_link)) * self.row_add,),
                )
    ...
    def init_walk(self):
        ...
        if not self.stationary:
            if self.gaussian:
                ...
                self.traj = np.random.choice(
                    np.array([-self.jump, self.jump]), (1, self.jump_rate)
                )
        else:
    ...
    def branch_traj(self):
        ...
        if not self.stationary:
            ...
            jump_add = int(self.count_link) * np.random.choice(
                np.array([-self.jump, self.jump]), (self.row_add,)
            )
            if not self.count_link:
                new_traj[self.rows : self.rows + self.row_add, : self.cols] = self.traj[
                    b_ind, :
                ].copy()
                new_traj[
                    self.rows + self.row_add : self.rows + 2 * self.row_add, : self.cols
                ] = self.traj[b_ind, :].copy()
            else:
                new_traj[self.rows : self.rows + self.row_add, : self.cols] = self.traj[
                    c_ind[b_ind], :
                ].copy()
                new_traj[
                    self.rows + self.row_add : self.rows + 2 * self.row_add, : self.cols
                ] = self.traj[c_ind[b_ind], :].copy()
            new_traj[
                : self.rows + 2 * (1 - int(self.count_link)) * self.row_add, -1
            ] = self.branch_choice()
            if self.count_link:
                new_traj[self.rows : self.rows + self.row_add, -1] = jump_add.copy()
                new_traj[
                    self.rows + self.row_add : self.rows + 2 * self.row_add, -1
                ] = jump_add.copy()
    ...
    def SP_evolution(self):
        ...
        self.selected_choices = self.traj[
            np.where(self.crossing == self.criteria)[0][
                np.argmin(np.where(self.crossing == self.criteria)[1])
            ],
            :,
        ]
        self.selected_path = self.evolved_traj[
            np.where(self.crossing == self.criteria)[0][
                np.argmin(np.where(self.crossing == self.criteria)[1])
            ],
            :,
        ...
        if not self.stationary:
            self.shortest_path = (
                np.where(self.crossing == self.criteria)[1].min() * self.jump**2
                + (1 - int(self.gaussian) - int(self.stationary)) * ct * self.jump**2
            )
            self.shortest_time = (
                np.where(self.crossing == self.criteria)[1].min()
                + (1 - int(self.gaussian) - int(self.stationary)) * ct
            )
        else:
            self.shortest_path = (
                np.where(self.crossing == self.criteria)[1].min() * self.jump**2
    ...
    def plot_and_fit_SPs(self):
        ...
        bin_range = np.linspace(
            np.min(np.array([SP.min(), self.SPs.min()])),
            np.max(np.array([SP.max(), self.SPs.max()])),
            self.bins,
        )
    ...
    def tau_SP_analysis(self, plot_analysis=True):
        ...
        """
        self.boxes = np.exp(
            np.linspace(np.log(self.box_min), np.log(self.box_max), self.log_points)
        )
        ...
        for i, box in enumerate(self.boxes):
            self.box = box
            self.SP_distribution()
            self.tau[i], self.sps[i], self.stsig[i], self.spsig[i] = (
                self.STs.mean(),
                self.SPs.mean(),
                self.STs.std(),
                self.SPs.std(),
        ...
        if plot_analysis:
            ...
            if self.count_link:
                np.savez(
                    "cl/tau_1D%s%s%s%s_rate_%.4f.npz"
                    % (
                        self.add_purge,
                        self.add_terminate,
                        self.add_den_terminate,
                ...
                np.savez(
                    "no_cl/tau_1D%s%s%s%s_rate_%.4f.npz"
                    % (
                        self.add_purge,
                        self.add_terminate,
                        self.add_den_terminate,
            ...
            ax1.plot(
                np.array([0, self.boxes.max()]),
                objective_log(np.array([0, self.boxes.max()]), a, b, e),
                "k--",
            )
            ...
            ax2.plot(
                np.array([0, self.boxes.max()]),
                objective_lin(np.array([0, self.boxes.max()]), a, b),
                "k--",
            )
    ...
    def c_rho_analysis(self, plot_analysis=True):
        ...
        if not self.count_link:
            if not self.terminate:
                np.savez(
                    "no_cl/%s1D_C1_rho.npz" % ("" if self.rho_max > 1.5 else "small_"),
                    rho_ref=ref_rho,
                    c1_ref=ref_c1,
                    rho_brw=self.rho_arr,
                    c1_brw=self.c_arr,
                    sig_brw=np.ones(4) if self.rho_max > 1.5 else self.sig_arr,
                )
            else:
                np.savez(
                    "no_cl/%sjust_term_1D_C1_rho.npz"
                    % ("" if self.rho_max > 1.5 else "small_"),
                    rho_ref=ref_rho,
                    c1_ref=ref_c1,
                    rho_brw=self.rho_arr,
                    c1_brw=self.c_arr,
            ...
            if not self.terminate:
                np.savez(
                    "cl/%sadj_1D_C1_rho.npz" % ("" if self.rho_max > 1.5 else "small_"),
                    rho_ref=ref_rho,
                    c1_ref=ref_c1,
                    rho_brw=self.rho_arr,
                    c1_brw=self.c_arr,
...
class Branched_2D_Walkers:
    ...
    def branch_choice(self):
        ...
        if not self.stationary:
            if self.gaussian:
                brancher = np.random.multivariate_normal(
                    np.zeros(2),
                    self.jump * np.identity(2),
                    (3 * self.rows, self.jump_rate),
                )
            elif self.lattice:
                brancher = np.array(
                    [
                        [random.choice(self.lat_jump) for i in range(self.jump_rate)]
                ...
                brancher[:, :, 0], brancher[:, :, 1] = self.jump * np.cos(
                    ang
                ), self.jump * np.sin(ang)
        else:
            if self.gaussian:
                brancher = np.random.multivariate_normal(
                    np.zeros(2),
                    self.jump * np.identity(2),
                    (self.rows + 2 * self.row_add,),
                )
            elif self.lattice:
                brancher = np.array(
                    [
                        [random.choice(self.lat_jump) for i in range(1)]
                        for j in range(self.rows + 2 * self.row_add)
                    ]
                )
            else:
                brancher = np.zeros(
                    (self.rows + 2 * (1 - int(self.count_link)) * self.row_add, 2)
                )
                ang = self.get_angles(
                    self.rows + 2 * (1 - int(self.count_link)) * self.row_add
                ).reshape(
    ...
    def count_nodes(self, r_in):
        ...
        self.final_loc = evolved_pos[:, -1, :]
        ct_ind = np.where(
            evolved_pos[:, :, 0] ** 2 + evolved_pos[:, :, 1] ** 2 < r_in**2
        )[0]
    ...
    def terminate_density(self):
        ...
        if del_rows:
            del_ind = np.where(
                self.final_loc[:, 0] ** 2 + self.final_loc[:, 1] ** 2 < self.r_ref**2
            )[0]
            self.traj = np.delete(self.traj, del_ind[:-1], axis=0)
        """
        self.calc_front()
    ...
    def crossed_barrier(self):
        ...
        self.evolved_traj = self.traj.cumsum(1)
        self.crossing = (
            (self.evolved_traj[:, :, 0] - self.box) ** 2
            + self.evolved_traj[:, :, 1] ** 2
            < 1
        ).astype(int)
        self.maxd = self.evolved_traj.max()
        return self.criteria in self.crossing
    ...
    def init_walk(self):
        ...
        if not self.stationary:
            if self.gaussian:
                self.traj = np.random.multivariate_normal(
                    np.zeros(2), self.jump * np.identity(2), (1, self.jump_rate)
                )
            elif self.lattice:
                self.traj = np.array(
                    [
                ...
                self.traj[:, :, 0], self.traj[:, :, 1] = self.jump * np.cos(
                    ang
                ), self.jump * np.sin(ang)
        else:
            self.ctlk = np.zeros(1)
            if self.gaussian:
                self.traj = np.random.multivariate_normal(
                    np.zeros(2), self.jump * np.identity(2), (1, 1)
                )
            elif self.lattice:
                self.traj = np.array(
                    [[random.choice(self.lat_jump) for i in range(1)] for j in range(1)]
                )
            else:
    ...
    def branch_traj(self):
        ...
        if not self.stationary:
            ...
            jump_ang = self.get_angles(self.row_add).reshape(
                -1,
            )
            jump_add = np.zeros((self.row_add, 2))
            jump_add[:, 0], jump_add[:, 1] = int(self.count_link) * self.jump * np.cos(
                jump_ang
            ), int(self.count_link) * self.jump * np.sin(jump_ang)
            new_traj[: self.rows, : self.cols, :] = self.traj.copy()
            if not self.count_link:
                new_traj[
                    self.rows : self.rows + self.row_add, : self.cols, :
                ] = self.traj[b_ind, :].copy()
                new_traj[
                    self.rows + self.row_add : self.rows + 2 * self.row_add,
                    : self.cols,
                    :,
                ] = self.traj[b_ind, :].copy()
            else:
    ...
    def plot_and_fit_SPs(self):
        ...
        if schematic:
            ...
            for i in range(20):
                plt.plot(
                    self.evolved_traj[random.choice(cc), :final_time, 0],
                    self.evolved_traj[random.choice(cc), :final_time, 1],
                    "k--",
                )
    ...
    def tau_SP_analysis(self, plot_analysis=True):
        ...
        if plot_analysis:
            ...
            if self.count_link:
                np.savez(
                    "cl/tau_2D%s%s%s%s_rate_%.4f.npz"
                    % (
                        self.add_purge,
                        self.add_terminate,
                        self.add_den_terminate,
                ...
                np.savez(
                    "no_cl/tau_2D%s%s%s%s_rate_%.4f.npz"
                    % (
                        self.add_purge,
                        self.add_terminate,
                        self.add_den_terminate,
    ...
    def c_rho_analysis(self, plot_analysis=True):
        ...
        if not self.count_link:
            if not self.terminate:
                np.savez(
                    "no_cl/%s2D_C1_rho.npz" % ("" if self.rho_max > 1.5 else "small_"),
                    rho_ref=ref_rho,
                    c1_ref=ref_c1,
                    rho_brw=self.rho_arr,
                    c1_brw=self.c_arr,
                ...
                np.savez(
                    "no_cl/%sjust_term_2D_C1_rho.npz"
                    % ("" if self.rho_max > 1.5 else "small_"),
                    rho_ref=ref_rho,
                    c1_ref=ref_c1,
                    rho_brw=self.rho_arr,
            ...
            if not self.terminate:
                np.savez(
                    "cl/%sadj_2D_C1_rho.npz" % ("" if self.rho_max > 1.5 else "small_"),
                    rho_ref=ref_rho,
                    c1_ref=ref_c1,
                    rho_brw=self.rho_arr,
                    c1_brw=self.c_arr,
...
class Branched_3D_Walkers:
    ...
    def branch_choice(self):
        ...
        if not self.stationary:
            if self.gaussian:
                ...
                brancher[:, :, 0], brancher[:, :, 1], brancher[:, :, 2] = (
                    self.jump * np.sin(phi) * np.cos(theta),
                    self.jump * np.sin(phi) * np.sin(theta),
                    self.jump * np.cos(phi),
                )
        else:
            if self.gaussian:
                brancher = np.random.multivariate_normal(
                    np.zeros(3),
                    self.jump**2 * self.dt * np.identity(3) / 3.0,
                    (self.rows + 2 * (1 - int(self.count_link)) * self.row_add,),
                )
            elif self.lattice:
                brancher = np.array(
                ...
                brancher = np.zeros(
                    (self.rows + 2 * (1 - int(self.count_link)) * self.row_add, 3)
                )
    ...
    def init_walk(self):
        ...
        if not self.stationary:
            if self.gaussian:
                self.traj = np.random.multivariate_normal(
                    np.zeros(3), self.jump**2 * np.identity(3), (1, self.jump_rate)
                )
            elif self.lattice:
                self.traj = np.array(
                    [
                ...
                self.traj[:, :, 0], self.traj[:, :, 1], self.traj[:, :, 2] = (
                    self.jump * np.sin(phi) * np.cos(theta),
                    self.jump * np.sin(phi) * np.sin(theta),
                    self.jump * np.cos(phi),
                )
            ...
            if self.gaussian:
                self.traj = np.random.multivariate_normal(
                    np.zeros(3), self.jump**2 * self.dt * np.identity(3) / 3.0, (1, 1)
                )
            elif self.lattice:
                self.traj = np.array(
                    [[random.choice(self.lat_jump) for i in range(1)] for j in range(1)]
    ...
    def branch_traj(self):
        ...
        if not self.stationary:
            ...
            if self.gaussian:
                jump_add = np.random.multivariate_normal(
                    np.zeros(3),
                    self.jump**2 * self.dt * np.identity(3) / 3.0,
                    (self.row_add,),
                )
                ...
                jump_add[:, 0], jump_add[:, 1], jump_add[:, 2] = (
                    int(self.count_link) * self.jump * np.sin(phi) * np.cos(theta),
                    int(self.count_link) * self.jump * np.sin(phi) * np.sin(theta),
                    int(self.count_link) * self.jump * np.cos(phi),
                )
            new_traj[: self.rows, : self.cols, :] = self.traj.copy()
            if not self.count_link:
                new_traj[
            ...
            if self.gaussian:
                ...
                to_jump = (
                    self.correlation * new_traj[: self.rows, -2, :]
                    + (1 - self.correlation**2) ** 0.5 * t_branch[: self.rows, :]
                )
                new_traj[: self.rows, -1, :] = (
                    np.diag(self.jump / np.linalg.norm(to_jump, axis=1)) @ to_jump
                )
            if self.count_link:
                new_traj[self.rows : self.rows + self.row_add, -1, :] = jump_add.copy()
                new_traj[
                ...
                if not self.gaussian:
                    new_traj[self.rows : self.rows + self.row_add, -1, :] = t_branch[
                        self.rows : self.rows + self.row_add, :
                    ]
                    no_jump = (
                        self.correlation
                        * t_branch[self.rows : self.rows + self.row_add, :]
                        + (1 - self.correlation**2) ** 0.5
                        * t_branch[
                            self.rows + self.row_add : self.rows + 2 * self.row_add, :
                        ]
                    )
                    new_traj[
                        self.rows + self.row_add : self.rows + 2 * self.row_add, -1, :
                    ] = (np.diag(self.jump / np.linalg.norm(no_jump, axis=1)) @ no_jump)
            self.traj = new_traj.copy()
    ...
    def SP_evolution(self):
        ...
        if not self.stationary:
            ...
            if self.gaussian:
                ...
                self.shortest_time = (
                    np.where(self.crossing == self.criteria)[1].min() * self.dt
                )
            else:
                self.shortest_path = (
                    np.where(self.crossing == self.criteria)[1].min() * self.jump**2
    ...
    def plot_and_fit_SPs(self):
        ...
        if self.count_link:
            np.savez(
                "cl/%s%s%s3D_hist.npz" % (self.add_jump, self.add_bbm, self.add_corr),
                sp_brw=self.STs,
                sp_cg=SP,
                sp_brw_dist=self.SPs,
            )
        else:
            np.savez(
                "no_cl/%s%s%s3D_hist.npz"
                % (self.add_jump, self.add_bbm, self.add_corr),
                sp_brw=self.STs,
                sp_cg=SP,
                sp_brw_dist=self.SPs,
            )
        bin_range = np.linspace(
            np.min(np.array([SP.min(), self.STs.min()])),
            np.max(np.array([SP.max(), self.STs.max()])),
            self.bins,
        )
    ...
    def tau_SP_analysis(self, plot_analysis=True):
        ...
        if self.count_link:
            np.savez(
                "cl/%s%s%sSP_brw.npz" % (self.add_jump, self.add_bbm, self.add_corr),
                sp10=sps[0],
                sp15=sps[1],
                sp20=sps[2],
                sp25=sps[3],
            ...
            np.savez(
                "no_cl/%s%s%sSP_brw.npz" % (self.add_jump, self.add_bbm, self.add_corr),
                sp10=sps[0],
                sp15=sps[1],
                sp20=sps[2],
                sp25=sps[3],
        ...
        if plot_analysis:
            ...
            if self.count_link:
                np.savez(
                    "cl/%s%s%stau_3D%s%s%s%s_rate_%.4f.npz"
                    % (
                        self.add_jump,
                        self.add_bbm,
                        self.add_corr,
                ...
                np.savez(
                    "no_cl/%s%s%stau_3D%s%s%s%s_rate_%.4f.npz"
                    % (
                        self.add_jump,
                        self.add_bbm,
                        self.add_corr,
        ...
        if not self.count_link:
            if not self.terminate:
                np.savez(
                    ...
                    c1_brw=self.c_arr,
                    sig_brw=np.ones(4) if self.rho_max > 1.5 else self.sig_arr,
                    c1_bar=np.ones(4) if self.rho_max > 1.5 else self.c_arr_bar,
                )
            else:
                np.savez(
                    "no_cl/%s%s%s%sjust_term_3D_C1_rho.npz"
                    % (
                        self.add_jump,
                        self.add_bbm,
                        self.add_corr,
...
def main():
    parser = argparse.ArgumentParser(
        description="Computing branched random walk analysis"
    )
    parser.add_argument(
        "--single_analysis",
        default=False,
        action="store_true",
        help="to plot distribution for a single rate",
    )
    parser.add_argument(
        "--tau_analysis",
        default=False,
        action="store_true",
        help="to plot Sp distributions as a function of box size",
    )
    parser.add_argument(
        "--crho_analysis", default=False, action="store_true", help="to plot C1 vs rho"
    )
    parser.add_argument(
        "--lattice", default=False, action="store_true", help="RW along a cubic lattice"
    )
    parser.add_argument(
        "--purge",
        default=False,
        action="store_true",
        help="Whether to purge backwardmost paths",
    )
    parser.add_argument(
        "--gaussian",
        default=False,
        action="store_true",
        help="uncorrelated gaussian jumps",
    )
    parser.add_argument(
        "--correlated",
        default=False,
        action="store_true",
        help="Whether steps are correlated",
    )
    parser.add_argument(
        "--stationary",
        default=False,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If reference CGMD should be plotted",
    )
    parser.add_argument(
        "--count_link",
        default=False,
        action="store_true",
        help="Choose whether weight of croslsink is to be calculated",
    )
    parser.add_argument(
        "--terminate",
        default=False,
        action="store_true",
        help="Whether to arbitrarily terminate path to ensure non-infinite chains",
    )
    parser.add_argument(
        "--density_terminate",
        default=False,
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.25 / 105,
        help="branching rate in units of inverse time",
    )
    parser.add_argument("--jump", type=float, default=1, help="jump length")
    parser.add_argument(
        "--correlation", type=float, default=0.4, help="extent of correlation"
    )
    parser.add_argument(
        "--box",
        type=float,
        default=65.5028,
        help="box length or periodic repeat length for completion",
    )
    parser.add_argument(
        "--num_paths",
        type=int,
        default=1000,
        help="number of paths for computing distribution",
    )
    parser.add_argument(
        "--bins", type=int, default=25, help="number of bins for SP distribution"
    )
    parser.add_argument(
        "--dim", type=int, default=1, help="Branching random-walk dimensionality"
    )
    parser.add_argument(
        "--box_min",
        type=int,
        default=10,
        help="minimum box size to use in tau analysis",
    )
    parser.add_argument(
        "--box_max",
        type=int,
        default=60,
        help="maximum box size to use in tau analysis",
    )
    parser.add_argument(
        "--rho_min", type=float, default=1.1, help="minimum rate to use in tau analysis"
    )
    parser.add_argument(
        "--rho_max", type=float, default=1.9, help="maximum rate to use in tau analysis"
    )
    parser.add_argument(
        "--time_discrete",
        type=float,
        default=3.0,
        help="discretization steps per unit time",
    )
    parser.add_argument(
        "--log_points",
        type=int,
        default=4,
        help="number of points on the log scale for deciding box sizes",
    )
    parser.add_argument(
        "--rho_points",
        type=int,
    ...
    if args.dim == 1:
        util = Branched_1D_Walkers(
            ...
            density_terminate=args.density_terminate,
            rho_min=args.rho_min,
            rho_max=args.rho_max,
            rho_points=args.rho_points,
            stationary=args.stationary,
            parallel=args.parallel,
            count_link=args.count_link,
            gaussian=args.gaussian,
        )
    elif args.dim == 2:
        util = Branched_2D_Walkers(
            rate=args.rate,
            jump=args.jump,
            box=args.box,
            num_paths=args.num_paths,
        ...
        util = Branched_3D_Walkers(
            ...
            box_min=args.box_min,
            box_max=args.box_max,
            time_discrete=args.time_discrete,
            log_points=args.log_points,
            lattice=args.lattice,
            purge=args.purge,
            terminate=args.terminate,
        ...
        raise ValueError(
            "Incorrect dimensionality specified by --dim. Try default --dim or --dim [2,3]"
        )
