...
from scipy.optimize import bisect, curve_fit, fsolve, root
...
class Theoretical_BRW:
    ...
    def m(self, x):
        ...
        return bisect(
            p, 1e-5 if self.dim < 2 else 1e-2 if self.dim > 2 else 1e-4, 100, xtol=0.001
        )
    ...
    def I(self, x):
        ...
        """
        if self.dim == 1:
            return self.m(x) * x - math.log(
                (math.e ** (self.m(x)) + math.e ** (-self.m(x))) / 2
            )
        else:
            return self.m(x) * x - math.log(
                integrate.quad(lambda y: self.g(self.m(x), y), -1, 1)[0]
                / (self.dim - 1)
            )
    ...
    def c1(self, rho):
        ...
        if self.method == "classical":
            ...
            if self.method == "termination":
                ...
                raise TypeError(
                    "The method of branching random walk is unrecognized. Assign either classical, termination or delayed_branching."
                )
            ...
            def q(y):
                ...
                """
                return self.I(y) - math.log(
                    0.5 * (1 - r)
                    + ((rho - 1) * (1 - 0.5 * r) + 0.25 * (1 - r) ** 2) ** (1 / 2)
                )

        return bisect(
            q,
            1e-5 if self.dim < 2 else 1e-2 if self.dim > 2 else 1e-4,
            0.999 if self.dim == 1 else 0.98,
            xtol=0.00001,
        )
    ...
    def FPT(self, rho):
        ...
        """
        return self.Lx / self.c1(rho) + (self.dim + 2) / 2 / self.c1(rho) / self.c2(
            rho
        ) * math.log(self.Lx)
...
class Theoretical_BBM:
    ...
    def c1(self, rho):
        ...
        def q(y):
            ...
            return self.I(y) - math.log(
                0.5 * (1 - self.rate)
                + ((rho - 1) * (1 - 0.5 * self.rate) + 0.25 * (1 - self.rate) ** 2)
                ** (1 / 2)
            )
    ...
    def FPT(self, v, r1):
        ...
        """
        return self.Lx / (v * math.sqrt(2 * math.log(r1))) + 5 * math.log(
            self.Lx / v
        ) / (4 * math.log(r1))
...
class Theoretical_GBRW:
    ...
    def FPT(self, v, rho):
        ...
        Returns:
            float: The calculated FPT value.
        """
        return (self.Lx / math.sqrt(v / 3)) / self.c1(rho) + 5 * (
            math.log(self.Lx / math.sqrt(v / 3))
        ) / (2 * self.c1(rho) * (self.c1(rho)))
...
def main():
    ...
    parser.add_argument(
        "--GBRW", default=False, action="store_true", help="to plot C1 vs rho"
    )
    parser.add_argument(
        "--rate",
        type=float,
        default=2.0 / 500.0,
        help="termination rate in units of inverse time",
    )
    parser.add_argument(
        "--dim", type=int, default=3, help="Branching random-walk dimensionality"
    )
    parser.add_argument(
        "--rho_range",
        type=str,
        default="small",
        help="minimum rate to use in tau analysis",
    )
    parser.add_argument(
        "--method",
        type=str,
        default="termination",
        help="maximum rate to use in tau analysis",
    )

    args = parser.parse_args()
    if args.BRW:
        util = Theoretical_BRW(
            rate=args.rate, dim=args.dim, rho_range=args.rho_range, method=args.method
        )
    elif args.BBM:
        util = Theoretical_BBM(
            rate=args.rate, dim=args.dim, rho_range=args.rho_range, method=args.method
        )
    elif args.GBRW:
        util = Theoretical_GBRW(
            rate=args.rate, dim=args.dim, rho_range=args.rho_range, method=args.method
        )
    else:
        raise ValueError(
