...
class Branched_3D_Walkers:
    ...
    def init_walk(self):
        ...
        if not self.stationary:
            ...
            if self.gaussian:
                self.traj = np.random.multivariate_normal(
                    np.zeros(3), self.cov * self.dt, (1, 1)
                )
            elif self.lattice:
                self.traj = np.array(
        ...
        if not self.stationary:
            ...
            if self.gaussian:
                jump_add = np.random.multivariate_normal(
                    np.zeros(3), self.cov * self.dt, (self.row_add,)
                )
            else:
        ...
        if reg_stuff:
            ...
            if not self.stationary:
                self.shortest_path = (
                    ...
                    + (1 - int(self.gaussian) - int(self.stationary))
                    * ct
                    * self.jump**2
                )
                self.shortest_time = (
                ...
                if self.gaussian:
                    ...
                    self.shortest_time = np.where(self.crossing == self.criteria)[
                        1
                    ].min()
        else:
    ...
    def SP_distribution(self):
        ...
        if schematic:
            ...
            np.savez(
                "AN_SP_%d_%s.npz" % (self.box, "small" if self.rate < 0.5 else "large"),
                **array_dict,
            )
            # plt.axis('equal')
            # plt.xlabel('x')
    ...
    def plot_and_fit_SPs(self):
        ...
        if self.count_link:
            np.savez(
                "cl/%s_%s%s%s3D_hist.npz"
                % (self.add_dep, self.add_jump, self.add_bbm, self.add_corr),
                sp_brw=self.STs,
                sp_cg=SP,
        ...
        if schematic:
            ...
            np.savez(
                "AN_SP_rest_%s.npz" % ("small" if self.rate < 0.5 else "large"),
                **array_dict,
            )
        print(self.STs.mean())
        plt.show()
    ...
    def tau_SP_analysis(self, plot_analysis=True):
        ...
        if plot_analysis:
            ...
            if self.count_link:
                np.savez(
                    "cl/%s_%s%s%stau_3D%s%s%s%s_rate_%.4f.npz"
                    % (
                        self.add_dep,
                        self.add_jump,
                        self.add_bbm,
                ...
                np.savez(
                    "no_cl/%s_%s%s%stau_3D%s%s%s%s_rate_%.4f.npz"
                    % (
                        self.add_dep,
                        self.add_jump,
                        self.add_bbm,
    ...
    def c_rho_analysis(self, plot_analysis=True):
        ...
        if not self.count_link:
            if not self.terminate:
                np.savez(
                    "no_cl/asymm_%s_%s%s%s%s3D_C1_rho.npz"
                    % (
                        self.add_dep,
                        self.add_jump,
                        self.add_bbm,
                    ...
                    c1_brw=self.c_arr,
                    sig_brw=np.ones(4) if self.rho_max > 3.5 else self.sig_arr,
                    c1_bar=np.ones(4) if self.rho_max > 3.5 else self.c_arr_bar,
                    boxes=self.boxes,
                    fpt=self.all_tau,
                    c2_brw=self.c2_arr,
                    c2_ref=ref_c2,
                ...
                np.savez(
                    "no_cl/asymm_%s_%s%s%s%sjust_term_3D_C1_rho.npz"
                    % (
                        self.add_dep,
                        self.add_jump,
                        self.add_bbm,
            ...
            if not self.terminate:
                np.savez(
                    "cl/asymm_%s_%s%s%s%sadj_3D_C1_rho.npz"
                    % (
                        self.add_dep,
                        self.add_jump,
                        self.add_bbm,
                ...
                np.savez(
                    "cl/asymm_%s_%s%s%s%sterm_3D_C1_rho.npz"
                    % (
                        self.add_dep,
                        self.add_jump,
                        self.add_bbm,
...
def main():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Whether the multivariate gaussian distribution for the jumps has dependency",
    )
    parser.add_argument(
        "--density_terminate",
        default=False,
    ...
    if args.dim == 1:
        raise ValueError(
            "asymmetric jump code has not been written for --dim 1. This script currently only suppoerts --dim 3."
        )
    elif args.dim == 2:
        raise ValueError(
            "asymmetric jump code has not been written for --dim 1. This script currently only suppoerts --dim 3."
