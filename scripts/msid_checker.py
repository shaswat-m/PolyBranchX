...
class Random_Walker:
    ...
    def generate_traj(self):
        ...
        self.traj[:, 0:1, 0], self.traj[:, 0:1, 1], self.traj[:, 0:1, 2] = (
            self.jump * np.sin(phi) * np.cos(theta),
            self.jump * np.sin(phi) * np.sin(theta),
            self.jump * np.cos(phi),
        )
        for i in range(1, self.nbead):
            to_jump = (
                self.correlation * self.traj[:, i - 1, :]
                + (1 - self.correlation**2) ** 0.5 * self.branch()
            )
    ...
    def compute_msid(self):
        ...
        for i in tqdm(range(1, self.nbead)):
            ...
            for j in range(self.nchain):
                ...
                if i == self.nbead - 1:
                    self.r2[j] = (
                        self.evolved_traj[j, :-i, 0] - self.evolved_traj[j, i:, 0]
                    )
